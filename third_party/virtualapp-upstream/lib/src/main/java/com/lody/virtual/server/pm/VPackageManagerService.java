...
/**
 * <p>维护虚拟环境中的应用包、组件和 ContentProvider 索引，并向客户端提供包管理查询。</p>
 *
 * @author Lody
 */
public class VPackageManagerService extends IPackageManager.Stub {
    ...
    /**
     * 按 authority 解析虚拟 ContentProvider，并在 microG 设置 Provider 的 authority 索引丢失时恢复索引。
     *
     * @param name Provider 的 authority。
     * @param flags 包管理查询标记。
     * @param userId 虚拟用户编号。
     * @return 可用的 Provider 信息；未找到或组件被禁用时返回 {@code null}。
     */
    @Override
    public ProviderInfo resolveContentProvider(String name, int flags, int userId) {
        ...
        if (provider == null && MICROG_SETTINGS_PROVIDER_AUTHORITY.equals(name)) {
            provider = repairMicrogSettingsProviderIndex();
        }
        if (provider != null && isEnabledLPr(provider.info, flags, userId)) {
    ...
    /**
     * 从 microG 已解析的 Provider 组件中恢复设置 Provider 的两张索引表。
     *
     * <p>部分包替换流程可能留下完整的 {@link VPackage}，但 authority 索引已经被旧包清理流程移除。
     * Google 登录读取 check-in 信息依赖这个 Provider，因此只对已验证的包名、类名和 authority
     * 三者同时匹配的组件执行恢复，避免把同名 authority 错误路由到其他应用。</p>
     *
     * @return 找到并恢复后的 Provider 组件；缓存中没有该组件时返回 {@code null}。
     */
    private VPackage.ProviderComponent repairMicrogSettingsProviderIndex() {
        synchronized (mPackages) {
            VPackage microgPackage = mPackages.get(MICROG_PACKAGE_NAME);
            if (microgPackage == null) {
                VLog.e(TAG, "Unable to repair microG settings provider: package is missing");
                return null;
            }

            for (VPackage.ProviderComponent candidate : microgPackage.providers) {
                if (!MICROG_SETTINGS_PROVIDER_CLASS_NAME.equals(candidate.info.name)
                        || !containsAuthority(candidate.info.authority,
                        MICROG_SETTINGS_PROVIDER_AUTHORITY)) {
                    continue;
                }

                // 同时恢复组件和 authority 索引，确保后续按组件及 URI 的查询结果保持一致。
                mProvidersByComponent.put(candidate.getComponentName(), candidate);
                synchronized (mProvidersByAuthority) {
                    mProvidersByAuthority.put(MICROG_SETTINGS_PROVIDER_AUTHORITY, candidate);
                }
                VLog.w(TAG, "Repaired missing microG settings provider authority index");
                return candidate;
            }

            VLog.e(TAG, "Unable to repair microG settings provider: parsed provider is missing, count=%d",
                    microgPackage.providers.size());
            return null;
        }
    }
     ...
     *
     * @param authorities Android 清单解析得到的 authority 列表。
     * @param expectedAuthority 需要精确匹配的 authority。
     * @return 包含目标 authority 时返回 {@code true}，否则返回 {@code false}。
     */
    private boolean containsAuthority(String authorities, String expectedAuthority) {
