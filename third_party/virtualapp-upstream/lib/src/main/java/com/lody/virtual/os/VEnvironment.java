...
public class VEnvironment {
    ...
    /**
     * 设置虚拟应用 APK 及其父目录的访问权限。
     *
     * <p>Android 14 及以上禁止从应用可写文件加载 dex。APK 在复制完成后必须变为只读，
     * 但父目录仍保持可写，以便下次更新时能够删除并替换旧 APK。</p>
     *
     * @param packageFile 已复制到虚拟应用目录的 APK 文件
     */
    public static void chmodPackageDictionary(File packageFile) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                int apkMode = Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE
                        // 只保留所有用户的读取权限，满足 Android 动态代码加载的不可写要求。
                        ? FileUtils.FileMode.MODE_IRUSR
                                | FileUtils.FileMode.MODE_IRGRP
                                | FileUtils.FileMode.MODE_IROTH
                        : FileUtils.FileMode.MODE_755;
                FileUtils.chmod(packageFile.getAbsolutePath(), apkMode);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static File[] getTFRoots(){
        File tfRoot = ensureCreated(new File(getContext().getFilesDir(), "virtual-tf"));
        File hostExternalFiles = ensureCreated(new File(tfRoot,
                "Android/data/" + getContext().getPackageName() + "/files"));
        return new File[]{hostExternalFiles};
    }
    /**
     * get TFCard root dir
