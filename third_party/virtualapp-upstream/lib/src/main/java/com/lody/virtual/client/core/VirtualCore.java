...
public final class VirtualCore {
    ...
    public void startup(Context context, SettingConfig config) throws Throwable {
        if (!isStartUp) {
            ...
            if (isServerProcess() || is64bitHelperProcess()) {
                ...
                try {
                    // 下载完成事件由系统 DownloadProvider 发送，必须允许系统 UID 投递。
                    BroadcastReceiverCompat.registerSystemReceiver(
                            context, mDownloadCompleteReceiver, filter);
                } catch (Throwable e) {
                    e.printStackTrace();
                }
            }
    ...
    private void detectProcessType() {
        ...
        } else if (VActivityManager.get().isAppProcess(processName) || isStubAppProcess(processName)) {
    ...
    private boolean isStubAppProcess(String processName) {
        if (processName == null || hostPkgName == null || !processName.startsWith(hostPkgName + ":p")) {
            return false;
        }
        int suffixStart = hostPkgName.length() + 2;
        if (suffixStart >= processName.length()) {
            return false;
        }
        for (int i = suffixStart; i < processName.length(); i++) {
            if (!Character.isDigit(processName.charAt(i))) {
                return false;
            }
        }
        return true;
    ...
    public boolean isOutsidePackageVisible(String pkg) {
        try {
            ...
            VLog.w(TAG, "isOutsidePackageVisible remote service unavailable pkg=%s error=%s", pkg, e);
            return false;
        }
    }

    /**
     * 判断指定包是否已经安装到 VirtualApp，并在服务 Binder 失效时自动重连一次。
     *
     * <p>这里只刷新 Binder 缓存，不卸载应用或清理虚拟数据目录，因此不会影响已经下载的
     * 游戏资源。限制为一次重试，避免服务持续不可用时阻塞宿主启动流程。</p>
     *
     * @param pkg 要检查的虚拟应用包名。
     * @return 已安装时返回 {@code true}，未安装时返回 {@code false}。
     */
    public boolean isAppInstalled(String pkg) {
        try {
            return getService().isAppInstalled(pkg);
        } catch (RemoteException firstError) {
            VLog.w(TAG, "isAppInstalled lost VirtualApp service; reconnecting once. pkg=%s error=%s",
                    pkg, firstError);
            // 同时清理两级缓存，否则新的 ServiceFetcher 仍可能被旧的 IAppManager 遮蔽。
            ...
            try {
                return getService().isAppInstalled(pkg);
            } catch (RemoteException retryError) {
                return VirtualRuntime.crash(retryError);
            }
        }
    }
    ...
    /**
     * 注册经过 VirtualApp 广播隔离处理的宿主内部接收器。
     *
    ...
    public void registerReceiver(Context context, Receiver receiver, IntentFilter filter){
        ...
        // 过滤器已被改写为宿主专属 Action，只允许同一应用 UID 的进程发送。
        BroadcastReceiverCompat.registerInternalReceiver(context, receiver, filter);
    }
