...
public class PackageParserCompat {
    ...
    private static final Object sUserState = API_LEVEL >= JELLY_BEAN_MR1 ? createPackageUserState() : null;
    ...
    private static Object createPackageUserState() {
        Object state = PackageUserState.ctor != null ? PackageUserState.ctor.newInstance() : null;
        if (state != null) {
            return state;
        }
        Class<?> type = PackageUserState.TYPE;
        if (type == null) {
            try {
                type = Class.forName("android.content.pm.PackageUserState");
            } catch (Throwable ignored) {
                return null;
            }
        }
        try {
            java.lang.reflect.Field defaultField = type.getDeclaredField("DEFAULT");
            defaultField.setAccessible(true);
            state = defaultField.get(null);
            if (state != null) {
                return state;
            }
        } catch (Throwable ignored) {
