...
public class ProviderHook implements InvocationHandler {
    ...
    protected void processArgs(Method method, Object... args) {
        ...
        String firstArgClass = firstArg.getClass().getName();
        if ("android.content.AttributionSource".equals(firstArgClass)) {
            AttributionSource original = (AttributionSource) firstArg;
            int hostUid = VirtualCore.get().getContext().getApplicationInfo().uid;
            if (hostUid <= 0) {
                hostUid = VirtualCore.get().myUid();
            }
            String hostPkg = VirtualCore.get().getHostPkg();
            AttributionSource rewritten = new AttributionSource.Builder(hostUid)
                    .setPackageName(hostPkg)
                    .build();
            Log.i("ProviderHook", "Rewrite AttributionSource method=" + method.getName()
                    + " oldUid=" + original.getUid()
                    + " oldPkg=" + original.getPackageName()
                    + " newUid=" + rewritten.getUid()
                    + " newPkg=" + rewritten.getPackageName());
            args[0] = rewritten;
        } else if ("query".equals(method.getName()) || "call".equals(method.getName())) {
            Log.i("ProviderHook", "Skip AttributionSource rewrite method=" + method.getName()
