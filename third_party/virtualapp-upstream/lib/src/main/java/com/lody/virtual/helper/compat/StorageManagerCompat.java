...
public class StorageManagerCompat {
    ...
    /**
     * Resolve the identity expected by IStorageManager#getVolumeList.
     *
     * Android 12 and older expose the first Binder argument as a UID. Android 13 changed the
     * same transaction to accept a userId. Passing the host UID to the newer interface makes the
     * system interpret (for example) 10624 as a cross-user request and reject ordinary apps with
     * INTERACT_ACROSS_USERS.
     */
    public static int getVolumeListCallerIdentity(int sdkInt, int realUid, int realUserId) {
        return sdkInt >= Build.VERSION_CODES.TIRAMISU ? realUserId : realUid;
    }
