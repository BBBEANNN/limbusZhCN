...
namespace FunctionDef {
    ...
    typedef jclass (*JNI_findClass)(JNIEnv *, const char *);
...
static struct {
    ...
    JNI_findClass orig_findClass;
    jmethodID method_onLimbusFindClass;
    bool limbus_findClass_hooked;
...
static bool isLimbusFindClassFallbackTarget(const char *name) {
    return startsWith(name, "com/google/firebase/")
           || (name != nullptr
               && strcmp(name, "com/google/android/gms/common/GoogleApiAvailability") == 0);
}

static bool makeWritable(void *addr) {
    long pageSize = sysconf(_SC_PAGESIZE);
    if (pageSize <= 0 || addr == nullptr) {
        return false;
    }
    uintptr_t page = reinterpret_cast<uintptr_t>(addr) & ~(static_cast<uintptr_t>(pageSize) - 1);
    return mprotect(reinterpret_cast<void *>(page), static_cast<size_t>(pageSize),
                    PROT_READ | PROT_WRITE) == 0;
}

static jclass new_limbus_FindClass(JNIEnv *env, const char *name) {
    jclass clazz = patchEnv.orig_findClass(env, name);
    if (clazz != nullptr || !isLimbusFindClassFallbackTarget(name)) {
        return clazz;
    }
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
    }
    if (patchEnv.method_onLimbusFindClass == nullptr) {
        return nullptr;
    }
    jstring javaName = env->NewStringUTF(name);
    jobject loaded = env->CallStaticObjectMethod(nativeEngineClass, patchEnv.method_onLimbusFindClass, javaName);
    if (javaName != nullptr) {
        env->DeleteLocalRef(javaName);
    }
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        ALOGE("FindClass >>> Limbus fallback threw for %s", name);
        return nullptr;
    }
    if (loaded != nullptr) {
        ALOGE("FindClass >>> Limbus fallback loaded %s", name);
        return static_cast<jclass>(loaded);
    }
    ALOGE("FindClass >>> Limbus fallback miss %s", name);
    return nullptr;
}
...
jstring new_nativeLoad(JNIEnv *env, jclass clazz, jstring _file, jobject classLoader, jobject _ld) {
    ...
    jstring result = patchEnv.orig_nativeLoad(env, clazz, _file, classLoader, _ld);
    if (result == nullptr) {
        const char *loaded_path = redirected_path != nullptr ? redirected_path : orig_path.c_str();
        ALOGE("nativeLoad >>> loaded path : %s", loaded_path != nullptr ? loaded_path : "(null)");
        onSoLoaded(loaded_path, reinterpret_cast<void *>(1));
        if (redirected_path != nullptr && orig_path.c_str() != nullptr && strcmp(redirected_path, orig_path.c_str()) != 0) {
            onSoLoaded(orig_path.c_str(), reinterpret_cast<void *>(1));
        }
    }
    return result;
}
...
void new_systemExit(JNIEnv *env, jclass clazz, jint code) {
    ...
    if (patchEnv.orig_systemExit) {
        patchEnv.orig_systemExit(env, clazz, code);
    }
}

void new_sendSignal(JNIEnv *env, jclass clazz, jint pid, jint sig) {
    jboolean blocked = env->CallStaticBooleanMethod(nativeEngineClass, patchEnv.method_onSendSignal, pid, sig, 0);
    if (blocked) {
        return;
    }
    if (patchEnv.orig_sendSignal) {
...
void new_sendSignalQuiet(JNIEnv *env, jclass clazz, jint pid, jint sig) {
    jboolean blocked = env->CallStaticBooleanMethod(nativeEngineClass, patchEnv.method_onSendSignal, pid, sig, 1);
    if (blocked) {
        return;
    }
    if (patchEnv.orig_sendSignalQuiet) {
...
void new_limbus_systemExit(JNIEnv *env, jclass clazz, jint code) {
    env->CallStaticVoidMethod(nativeEngineClass, patchEnv.method_onSystemExit, code);
}

void new_limbus_sendSignal(JNIEnv *env, jclass clazz, jint pid, jint sig) {
    jboolean blocked = env->CallStaticBooleanMethod(nativeEngineClass, patchEnv.method_onSendSignal, pid, sig, 0);
    if (!blocked) {
        kill(pid, sig);
    }
...
void new_limbus_sendSignalQuiet(JNIEnv *env, jclass clazz, jint pid, jint sig) {
    jboolean blocked = env->CallStaticBooleanMethod(nativeEngineClass, patchEnv.method_onSendSignal, pid, sig, 1);
    if (!blocked) {
        kill(pid, sig);
    }
...
void hookLimbusProcessExit(JNIEnv *env) {
    ...
    patchEnv.method_onSendSignal = env->GetStaticMethodID(nativeEngineClass,
                                                          "onSendSignal",
                                                          "(III)Z");
    if (!patchEnv.method_onSystemExit || !patchEnv.method_onSendSignal) {
        ALOGE("Limbus process exit hook missing Java callbacks");
        return;
    }
    ...
    if (runtimeClass) {
        JNINativeMethod runtimeMethods[] = {
                {"nativeExit", "(I)V", (void *) new_limbus_systemExit},
        };
        if (env->RegisterNatives(runtimeClass, runtimeMethods, 1) != 0) {
            ALOGE("Limbus process exit hook failed to register Runtime.nativeExit");
            env->ExceptionClear();
        }
    } else {
        ALOGE("Limbus process exit hook failed to find java/lang/Runtime");
        env->ExceptionClear();
    }

    jclass processClass = env->FindClass("android/os/Process");
    if (processClass) {
        JNINativeMethod processMethods[] = {
                {"sendSignal", "(II)V", (void *) new_limbus_sendSignal},
                {"sendSignalQuiet", "(II)V", (void *) new_limbus_sendSignalQuiet},
        };
        if (env->RegisterNatives(processClass, processMethods, 2) != 0) {
            ALOGE("Limbus process exit hook failed to register Process signal methods");
            env->ExceptionClear();
        }
    } else {
        ALOGE("Limbus process exit hook failed to find android/os/Process");
        env->ExceptionClear();
    }

    installed = true;
    ALOGE("Limbus process exit hook installed");
}

void hookLimbusFindClass(JNIEnv *env) {
    if (patchEnv.limbus_findClass_hooked || env == nullptr) {
        return;
    }
    patchEnv.method_onLimbusFindClass = env->GetStaticMethodID(
            nativeEngineClass,
            "findLimbusClassFromNative",
            "(Ljava/lang/String;)Ljava/lang/Class;");
    if (patchEnv.method_onLimbusFindClass == nullptr) {
        if (env->ExceptionCheck()) {
            env->ExceptionClear();
        }
        ALOGE("FindClass >>> unable to locate Limbus Java fallback method");
        return;
    }
    JNINativeInterface *table = const_cast<JNINativeInterface *>(env->functions);
    if (table == nullptr || table->FindClass == nullptr) {
        ALOGE("FindClass >>> invalid JNI function table");
        return;
    }
    if (!makeWritable(&table->FindClass)) {
        ALOGE("FindClass >>> unable to make JNI table writable");
        return;
    }
    patchEnv.orig_findClass = table->FindClass;
    table->FindClass = new_limbus_FindClass;
    patchEnv.limbus_findClass_hooked = true;
    ALOGE("FindClass >>> installed Limbus Firebase fallback hook");
}
