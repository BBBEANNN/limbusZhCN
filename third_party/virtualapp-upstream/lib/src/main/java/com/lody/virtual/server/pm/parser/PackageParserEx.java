...
/**
 * 负责把 Android 原生包解析结果转换为 VirtualApp 可持久化的虚拟包模型。
 *
 * @author Lody
 */
public class PackageParserEx {
    ...
    /**
     * 解析 APK，并生成 VirtualApp 使用的包缓存对象。
     *
     * @param packageFile 待解析的 APK 文件
     * @return 已转换的虚拟包信息
     * @throws Throwable 原生包解析或虚拟包转换失败时抛出的异常
     */
    public static VPackage parsePackage(File packageFile) throws Throwable {
        ...
        if (BuildCompat.isQ()) {
            setParserCallbackIfSupported(parser);
        }
        ...
        if (p.requestedPermissions.contains("android.permission.FAKE_PACKAGE_SIGNATURE")
            ...
            signatureOverride = new Signature[]{new Signature(sig)};
            try {
                // 旧系统仍可把伪签名写回原生解析对象，保留该行为以兼容既有调用。
                buildSignature(p, signatureOverride);
            } catch (Throwable error) {
                // Android 16 的私有 SigningDetails 字段可能发生变化；虚拟包缓存仍会使用
                // signatureOverride，因此这里不能让反射失败中断整个 APK 安装。
                VLog.w(TAG, "Unable to update platform SigningDetails for %s\n%s",
                        p.packageName, VLog.getStackTraceString(error));
            }
        ...
        return buildPackageCache(p, signatureOverride);
    }

    private static void setParserCallbackIfSupported(PackageParser parser) {
        try {
            Constructor<PackageParser.CallbackImpl> constructor =
                    PackageParser.CallbackImpl.class.getDeclaredConstructor(PackageManager.class);
            constructor.setAccessible(true);
            parser.setCallback(constructor.newInstance(VirtualCore.getPM()));
        } catch (Throwable ignored) {
            VLog.w(TAG, "PackageParser.CallbackImpl(PackageManager) unavailable; parse without callback.");
        }
    }
    ...
    private static VPackage buildPackageCache(
        ...
        // 伪签名已经由宿主验证并明确授权时，优先写入 VirtualApp 自己的包缓存。
        // 这样不再依赖 Android 私有 SigningDetails 的字段布局。
        cache.mSignatures = signatureOverride != null ? signatureOverride : getSignature(p);
    ...
    private static boolean initApplicationAsUser(ApplicationInfo ai, int userId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            boolean has64BitAbi = Build.SUPPORTED_64_BIT_ABIS.length > 0;
            boolean has32BitAbi = Build.SUPPORTED_32_BIT_ABIS.length > 0;
            if (is64bit) {
                if (has64BitAbi) {
                    ApplicationInfoL.primaryCpuAbi.set(ai, Build.SUPPORTED_64_BIT_ABIS[0]);
                }
                if (ps.flag == PackageSetting.FLAG_RUN_BOTH_32BIT_64BIT && has32BitAbi) {
                ...
                if (has32BitAbi) {
                    ...
                    if (ps.flag == PackageSetting.FLAG_RUN_BOTH_32BIT_64BIT && has64BitAbi) {
                        ApplicationInfoL.secondaryCpuAbi.set(ai, Build.SUPPORTED_64_BIT_ABIS[0]);
                    }
                } else if (has64BitAbi) {
                    // 新款 arm64-only 设备不再提供 32 位 ABI；旧代码直接访问空数组会让服务进程崩溃。
                    // 对无原生库或“双 ABI”虚拟包回退到设备可执行的 64 位 ABI。
                    ApplicationInfoL.primaryCpuAbi.set(ai, Build.SUPPORTED_64_BIT_ABIS[0]);
                }
            }
        }
