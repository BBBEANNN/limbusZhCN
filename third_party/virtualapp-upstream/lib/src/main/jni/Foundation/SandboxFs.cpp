...
static bool is_limbus_translation_redirect(const char *orig_path, const char *new_path) {
    return orig_path != NULL
           && new_path != NULL
           && strstr(orig_path, "com.ProjectMoon.LimbusCompany") != NULL
           && strstr(orig_path, "/Localize/") != NULL
           && strstr(new_path, "com.example.limbuszhcn") != NULL
           && strstr(new_path, "/translation-cache/") != NULL;
}

int add_replace_item(const char *orig_path, const char *new_path) {
    const bool limbus_translation = is_limbus_translation_redirect(orig_path, new_path);
    if (limbus_translation) {
        for (int i = 0; i < replace_item_count; ++i) {
            ReplaceItem &existing = replace_items[i];
            if (strcmp(existing.orig_path, orig_path) == 0) {
                free(existing.new_path);
                existing.new_path = strdup(new_path);
                existing.new_size = strlen(new_path);
                existing.is_folder = (orig_path[strlen(orig_path) - 1] == '/');
