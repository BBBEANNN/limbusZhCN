...
/* package */ class ActivityStack {
    ...
    int startActivityLocked(int userId, Intent intent, ActivityInfo info, IBinder resultTo, Bundle options,
        ...
        Log.i("LimbusVA", "ActivityStack.startActivityLocked userId=" + userId
    ...
    private String fetchStubActivity(int vpid, ActivityInfo targetInfo) {
        ...
        try {
            if (R_Hide.styleable.Window != null
                    && R_Hide.styleable.Window_windowIsTranslucent != null
                    && R_Hide.styleable.Window_windowIsFloating != null
                    && R_Hide.styleable.Window_windowShowWallpaper != null) {
        ...
        Log.i("LimbusVA", "ActivityStack.fetchStubActivity vpid=" + vpid
                + " target=" + describeActivityInfoForLimbus(targetInfo)
                + " isFloating=" + isFloating
                + " isTranslucent=" + isTranslucent
                + " showWallpaper=" + showWallpaper
                + " isDialogStyle=" + isDialogStyle);
        if (isDialogStyle) {
    ...
    private Intent startActivityProcess(int userId, ActivityRecord targetRecord, Intent intent, ActivityInfo info, int callingUid, int callingPid) {
        ...
        Log.i("LimbusVA", "ActivityStack.startActivityProcess userId=" + userId
                + " target=" + describeActivityInfoForLimbus(info)
                + " callingUid=" + callingUid
                + " callingPid=" + callingPid
                + " sourceIntent=" + describeIntentForLimbus(intent));
        ...
        if (targetApp == null) {
            Log.w("LimbusVA", "ActivityStack.startActivityProcess targetApp=null target=" + describeActivityInfoForLimbus(info));
            return null;
        }
        Log.i("LimbusVA", "ActivityStack.startActivityProcess targetApp process=" + targetApp.processName
                + " pid=" + targetApp.pid
                + " vpid=" + targetApp.vpid
                + " vuid=" + targetApp.vuid
                + " is64bit=" + targetApp.is64bit);
    ...
    Intent getStartStubActivityIntentInner(Intent intent, boolean is64bit, int vpid, int userId, ActivityRecord targetRecord, ActivityInfo info) {
        ...
        Log.i("LimbusVA", "ActivityStack.stubIntent userId=" + userId
                + " vpid=" + vpid
                + " is64bit=" + is64bit
                + " target=" + describeActivityInfoForLimbus(info)
                + " realIntent=" + describeIntentForLimbus(intent)
                + " stubIntent=" + describeIntentForLimbus(targetIntent));
        return targetIntent;
    }
