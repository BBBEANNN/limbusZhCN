...
//        addService(ServiceManagerNative.CKMSSAFEKEY, VSafekeyCkmsManagerService.get());
    ...
    private void clearOldTask(Context context) {
        if (shouldPreserveRunningVirtualClients(context)) {
            VLog.w(TAG, "skip invalid task cleanup for host %s", context.getPackageName());
            return;
        }
...
//                        VLog.w(TAG, "unknown=%s", taskInfo.baseIntent);
    ...
    private void clearOldProcess(Context context) {
        if (shouldPreserveRunningVirtualClients(context)) {
            VLog.w(TAG, "skip invalid process cleanup for host %s", context.getPackageName());
            return;
        }
        ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
    ...
    private boolean shouldPreserveRunningVirtualClients(Context context) {
        return "com.example.limbuszhcn".equals(context.getPackageName());
    }
