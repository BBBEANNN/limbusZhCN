...
public class BroadcastSystem {
    ...
    public void startApp(VPackage p) {
        ...
        for (VPackage.ActivityComponent receiver : p.receivers) {
            ...
            // 组件 Action 由 VirtualApp 生成，只允许宿主内部广播命中该接收器。
            BroadcastReceiverCompat.registerInternalReceiver(
                    mContext, r, componentFilter, null, mScheduler);
            receivers.add(r);
            ...
            for (VPackage.ActivityIntentInfo ci : receiver.intents) {
                ...
                // 原始广播 Action 已改写为宿主专属 Action，外部应用无需直接投递。
                BroadcastReceiverCompat.registerInternalReceiver(
                        mContext, r, cloneFilter, null, mScheduler);
            }
        }
    }
