...
public class VActivityManager {
    ...
    public IntentSenderData getIntentSenderByBridgeKey(String bridgeKey) {
        try {
            return getService().getIntentSenderByBridgeKey(bridgeKey);
        } catch (RemoteException e) {
            return VirtualRuntime.crash(e);
        }
    }
    ...
    public List<AppRunningProcessInfo> getRunningAppProcesses(String packageName, int userId) {
        try {
            ...
            List<AppRunningProcessInfo> fallback = getCurrentLimbusRunningProcessFallback(packageName);
            if (fallback != null) {
                VLog.w("VActivityManager", "getRunningAppProcesses fallback for Limbus after remote failure: " + e);
                return fallback;
            }
            return VirtualRuntime.crash(e);
        }
    ...
    private List<AppRunningProcessInfo> getCurrentLimbusRunningProcessFallback(String packageName) {
        if (!"com.ProjectMoon.LimbusCompany".equals(packageName)) {
            return null;
        }
        VClient client = VClient.get();
        if (!"com.ProjectMoon.LimbusCompany".equals(client.getCurrentPackage())) {
            return null;
        }
        ClientConfig config = client.getClientConfig();
        AppRunningProcessInfo info = new AppRunningProcessInfo();
        info.pid = Process.myPid();
        info.vuid = VClient.get().getVUid();
        info.vpid = config != null ? config.vpid : 0;
        info.packageName = packageName;
        info.processName = config != null && !TextUtils.isEmpty(config.processName)
                ? config.processName
                : packageName;
        info.pkgList.add(packageName);
        List<AppRunningProcessInfo> result = new ArrayList<>(1);
        result.add(info);
        return result;
    }
