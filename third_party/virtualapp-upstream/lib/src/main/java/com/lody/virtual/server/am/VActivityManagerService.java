...
import com.lody.virtual.helper.compat.BroadcastReceiverCompat;
...
public class VActivityManagerService extends IActivityManager.Stub {
    ...
    public static void systemReady() {
        ...
        // 配置变化由 Android 系统发送，需要允许系统 UID 投递给动态接收器。
        BroadcastReceiverCompat.registerSystemReceiver(
                VirtualCore.get().getContext(), new BroadcastReceiver() {
            @Override
...
//                    finish all
    ...
    public int startActivity(Intent intent, ActivityInfo info, IBinder resultTo, Bundle options, String resultWho, int requestCode, int userId) {
        synchronized (this) {
            Log.i("LimbusVA", "VAMS.startActivity userId=" + userId
                    + " callingUid=" + VBinder.getCallingUid()
                    + " callingPid=" + VBinder.getCallingPid()
                    + " info=" + describeActivityInfoForLimbus(info)
                    + " resultTo=" + resultTo
                    + " requestCode=" + requestCode
                    + " intent=" + describeIntentForLimbus(intent));
    ...
    @Override
    public IntentSenderData getIntentSenderByBridgeKey(String bridgeKey) {
        if (bridgeKey != null) {
            synchronized (mIntentSenderMap) {
                for (IntentSenderData data : mIntentSenderMap.values()) {
                    if (bridgeKey.equals(data.bridgeKey)) {
                        return data;
                    }
                }
            }
...
//            }
     ...
     * 查找可供新虚拟进程使用的宿主进程槽位。
     *
     * <p>VAMS 服务端可能被厂商内存回收单独杀死，而 {@code :pN} 游戏进程仍然存活。
     * 因此除了检查服务端内存中的 ProcessRecord，还要检查系统正在运行的宿主进程名，
     * 避免把新服务分配到已被游戏占用的槽位。</p>
     *
     * @param is64bit 新虚拟进程是否使用 64 位宿主引擎。
     * @return 可用的 vpid；无空闲槽位时返回 {@code -1}。
     */
    public int queryFreeStubProcess(boolean is64bit) {
        List<ActivityManager.RunningAppProcessInfo> runningProcesses = am == null
                ? Collections.<ActivityManager.RunningAppProcessInfo>emptyList()
                : am.getRunningAppProcesses();
        if (runningProcesses == null) {
            runningProcesses = Collections.emptyList();
        }
        synchronized (mProcessLock) {
            for (int vpid = 0; vpid < StubManifest.STUB_COUNT; vpid++) {
                ...
                if (isStubProcessAlive(vpid, is64bit, runningProcesses)) {
                    VLog.w(TAG, "Skip live stub process missing from VAMS records: vpid=%d is64bit=%s",
                            vpid, is64bit);
                    continue;
                }
                return vpid;
    ...
    /**
     * 判断指定槽位的宿主进程是否仍被 Android 保留。
     *
     * @param vpid 待检查的虚拟进程槽位。
     * @param is64bit 是否查询 64 位宿主引擎。
     * @param runningProcesses Android 当前可见的运行进程列表。
     * @return 存在与槽位进程名完全匹配的进程时返回 {@code true}。
     */
    private boolean isStubProcessAlive(int vpid, boolean is64bit,
                                       List<ActivityManager.RunningAppProcessInfo> runningProcesses) {
        String stubPackageName = StubManifest.getStubPackageName(is64bit);
        if (TextUtils.isEmpty(stubPackageName)) {
            return false;
        }
        String expectedProcessName = stubPackageName + ":p" + vpid;
        for (ActivityManager.RunningAppProcessInfo process : runningProcesses) {
            if (process != null && TextUtils.equals(expectedProcessName, process.processName)) {
                return true;
            }
        }
        return false;
    ...
    private static String describeIntentForLimbus(Intent intent) {
        if (intent == null) {
            return "null";
        }
    ...
    private static String describeActivityInfoForLimbus(ActivityInfo info) {
        if (info == null) {
            return "null";
        }
